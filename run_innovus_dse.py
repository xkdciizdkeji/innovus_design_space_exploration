...
def simulated_annealing(case, boundary, core_utilization, max_iterations=100, initial_temperature=1.0, cooling_rate=0.99, min_temperature=0.01, high_temp_ratio=0.7, low_temp_ratio=0.3):
    ...
    with open(log_file, "w") as f:
        ...
        f.write("\n")
        f.write("iteration,modification_type,total_net_length,total_via_count,runtime,loss,loss_change,temperature,accepted,num_groups\n")
    ...
    while iteration <= max_iterations and temperature >= min_temperature:
        ...
        print(f"本次迭代将修改 {num_groups} 个组")
        
        # 生成新的约束文件
        new_constraint_file = f"constraint/{case}__{boundary}__{core_utilization}__{iteration}.txt"
        modification_type = generate_random_constraint(current_constraint_file, new_constraint_file, None, shift_distance=1.0, num_groups=num_groups)
        ...
        with open(log_file, "a") as f:
            ...
            f.write(f"{iteration},{mod_types_str},{current_data['total_net_length']},{current_data['total_via_count']},{current_data['total_runtime']},{loss_current},{loss_change},{temperature},{accept},{num_groups}\n")
